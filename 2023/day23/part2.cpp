...
void Graph::make_bidirectional(){
  ...
  for(int y=0; y<n_y; y++){
    for(int x=0; x<n_x; x++){
      ...
      for(Edge & edge : node->edges)
        edge.node->reversed_edges.push_back(Edge(node, edge.weight));
  ...
  for(int y=0; y<n_y; y++){
    for(int x=0; x<n_x; x++){
      ...
      if(node->reversed_edges.size() == 0)
        continue;

      for(Edge & edge : node->reversed_edges)
...
void Graph::find_path(Node * node, int path_length, set<Node *> visited_nodes){
  ...
  for(Edge & edge : node->edges)
    find_path(edge.node, path_length + edge.weight, visited_nodes);
