...
#          print "fold G", x, y, target, turn[coord] #XXX
        ...
        offset = ((i % 2 ? x : y) - 1) % side_length
        target = neighbor(i, -2*side_length + 1, x SUBSEP y)
        target = neighbor((i + (mirror ? 3 : 1)) % 4, ((i + (mirror ? 3 : 1)) % 4 > 1 ? -2*(side_length-offset) + 1 : -2*offset - 1) + 5*side_length, target)
        if(edge[(i + 2) % 4, target] == 1 && map[target] ~ "\\.|#" && map[neighbor((i + 2) % 4, 1, target)] !~ "\\.|#"){
          edge[coord] = target
          edge[(i + 2) % 4, target] = x SUBSEP y
#          print "fold H"mirror, x, y, target, turn[coord] #XXX
          Map[x, y] = "H" #XXX
          Map[target] = "H" #XXX
        }
      }
        ...
        target = neighbor(i, -3*side_length + 1, x SUBSEP y)
        target = neighbor((i + (mirror ? 3 : 1)) % 4, 3*side_length, target)
        if(edge[(i + 2) % 4, target] == 1 && map[target] ~ "\\.|#" && map[neighbor(i, -1, target)] !~ "\\.|#"){
          edge[coord] = target
          edge[(i + 2) % 4, target] = x SUBSEP y
#          print "fold I", x, y, target, turn[coord] #XXX
          Map[x, y] = "I" #XXX
          Map[target] = "I" #XXX
        }
      }
